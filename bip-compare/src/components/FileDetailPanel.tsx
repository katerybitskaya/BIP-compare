...
> = {
  ...
  removed: {
    ...
    tone: 'bg-slate-500/10 text-slate-300 ring-1 ring-slate-400/20',
  },
};
...
export default function FileDetailPanel({ file, onClose }: FileDetailPanelProps) {
  if (!file) {
    return (
      <aside className="hidden w-full shrink-0 rounded-2xl border border-white/10 bg-white/[0.03] p-6 shadow-lg shadow-black/20 backdrop-blur lg:flex lg:w-96 lg:flex-col lg:items-center lg:justify-center">
  ...
  return (
    <aside className="flex w-full shrink-0 flex-col rounded-2xl border border-white/10 bg-white/[0.03] shadow-lg shadow-black/20 backdrop-blur lg:w-96">
      <div className="flex items-center justify-between border-b border-white/5 p-4">
        <h2 className="text-sm font-semibold text-slate-100">Szczegóły pliku</h2>
        <button
          type="button"
          onClick={onClose}
          className="flex h-7 w-7 items-center justify-center rounded-lg text-slate-500 hover:bg-white/5 hover:text-slate-200"
      ...
      <div className="space-y-5 p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-0.5 break-all text-xs text-slate-500">{file.path}</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <dl className="space-y-2 text-xs">
              <div className="flex items-center justify-between">
                <dt className="text-slate-400">Status HTTP</dt>
                <dd className="font-medium"><HttpBadge code={file.oldHttp} /></dd>
              </div>
              <div className="flex items-center justify-between">
                <dt className="text-slate-400">Rozmiar</dt>
                <dd className="font-medium text-slate-200">{formatSize(file.oldSizeKb)}</dd>
              </div>
              <div className="flex items-center justify-between gap-2">
                <dt className="shrink-0 text-slate-400">Typ</dt>
                <dd className="truncate font-medium text-slate-200" title={file.oldType}>
                  {file.oldType.split('/')[1] ?? file.oldType}
                </dd>
              </div>
              <div className="flex items-center justify-between">
                <dt className="text-slate-400">Pobranie</dt>
                <dd className={`font-medium ${file.oldDownloadOk ? 'text-emerald-400' : 'text-rose-400'}`}>
                  {file.oldDownloadOk ? 'Działa' : 'Niedostępne'}
                </dd>
              </div>
              <div>
                <dt className="text-slate-400">Ostatnia modyfikacja</dt>
                <dd className="mt-0.5 font-medium text-slate-200">{file.oldModified ?? '–'}</dd>
              </div>
            </dl>
          </div>
          ...
          <div>
            ...
            <dl className="space-y-2 text-xs">
              ...
              <div className="flex items-center justify-between">
                <dt className="text-slate-400">Rozmiar</dt>
                <dd className="font-medium text-slate-200">{formatSize(file.newSizeKb)}</dd>
              </div>
              <div className="flex items-center justify-between gap-2">
                <dt className="shrink-0 text-slate-400">Typ</dt>
                <dd className="truncate font-medium text-slate-200" title={file.newType}>
              ...
              <div>
                <dt className="text-slate-400">Ostatnia modyfikacja</dt>
                <dd className="mt-0.5 font-medium text-slate-200">{file.newModified ?? '–'}</dd>
              </div>
            </dl>
          </div>
        </div>
